...
@section('title','Post Edit: '.$post->title)
...
@section('content')
  ...
  @component('admin.component.breadcrumb')
    @slot('title')
      <h5 class="font-medium text-uppercase mb-0">Post Edit</h5>
    @endslot
                              ...
                              <div class="row pt-3">
                                  <div class="col-md-12">
                                      <div class="form-group {{$errors->has('title') ? 'has-danger' : ''}}">
                                          ...
                                          <input type="text" id="lastName" name="title" class="form-control" style="color:#000;" placeholder="Title" value="{{$post->title}}">
                                          @if($errors->has('title'))
                                  ...
                                  <div class="col-md-12">
                                      <div class="form-group {{$errors->has('short_description') ? 'has-danger' : ''}}">
                                          ...
                                          <textarea class="form-control" rows="6" name="short_description" style="color:#000;" placeholder="Text Here...">{{$post->short_description}}</textarea>
                    ...
                    <div class="col-md-12">
                      <div class="form-group {{$errors->has('publish') ? 'has-danger' : ''}}">
                          ...
                          <div class="custom-control custom-radio">
                              <input type="radio" class="custom-control-input" name="publish" id="publish_now"  {{$post->published_at <= Carbon\Carbon::now() ? 'checked': ''}}  value="1" >
                              <label class="custom-control-label" for="publish_now">Now</label>
                          </div>
                      </div>
                      <div class="form-check form-check-inline">
                          <div class="custom-control custom-radio">
                              <input type="radio" class="custom-control-input" name="publish" id="publish_schedule" {{$post->published_at > Carbon\Carbon::now() ? 'checked': ''}}   value="2">
                    ...
                    <div class="col-md-12">
                        <div class="form-group {{$errors->has('publist_time') ? 'has-danger' : ''}}">
                            <label class="control-label">Post Schedule Date & Time</label>
                              @php
                                $date_time=$post->published_at->format('Y-m-d H:i');
                                $dateTime=str_replace(' ','T',$date_time);
                              @endphp
                            <input type="datetime-local" name="publist_time" class="form-control" value="{{$dateTime}}">
                      ...
                      <div class="form-group {{$errors->has('category') ? 'has-danger' : ''}}">
                          <select class="select2 form-control" name="category[]" multiple="multiple" style="height: 36px;width: 100%;">
                            @foreach($category as $data)
                              <option value="{{$data->id}}" @foreach($post->categorys as $cat) {{$data->id == $cat->id ? 'selected' : ''}} @endforeach>{{$data->name}}</option>
                            @endforeach
                          </select>
                    ...
                    <div class="col-md-12">
                      <div class="form-group {{$errors->has('tag') ? 'has-danger' : ''}}">
                          <select class="select2 form-control" name="tag[]" multiple="multiple" style="height: 36px;width: 100%;">
                            @foreach($tags as $tag)
                              <option value="{{$tag->id}}" @foreach($post->tags as $posttag) {{$posttag->id == $tag->id ? 'selected' : ''}} @endforeach>{{$tag->name}}</option>
                            @endforeach
                          </select>
                          @if($errors->has('tag'))
            ...
            <div class="col-12">
              <div class="material-card card">
                ...
                <div class="card-body">
                  ...
                  <img src="{{$post->image_url}}" width="205" alt="">
                </div>
              </div>
            </div>
          </div>
